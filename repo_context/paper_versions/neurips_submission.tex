...
\usepackage{neurips_2024}   % [preprint] for arXiv, [final] for camera-ready
...
\textbf{Research gap.}
Prior LLM--brain alignment work has focused on language tasks via RSA or encoding
models~\citep{caucheteux2022,schrimpf2021,li2023b,huth2016,li2023a}.
Emotion geometry has been studied in affective computing~\citep{russell1980,koelstra2012}
but the void--belt--margin structure and its cross-system ambiguity gradient have not
...
\citet{ismayilzada2026} found alignment during creative thinking (preprint).
...
well above the 5-class chance level of 0.20 (sample-size comparison in
Appendix~\ref{app:sample-size}).
...
includes Qwen3-1.7B fine-tuned~\citep{qwen2025} (PCA-projected to 768D) as a third
architecture to test whether the compaction--distribution finding generalises beyond
the two primary model families.
...
With $K\!=\!3$ emotions, RDMs have 1 degree of freedom ($p\!\approx\!0.21$ for $r\!=\!-0.88$;
formally non-significant); load-bearing quantities are sign and noise-ceiling contrast.
The 11D result (Section~\ref{sec:inversion}) provides stronger support via a fully
...
\textbf{The Void.}
No data points exist within approximately 0.375--0.8125 normalised distance units of
any emotion centroid.
Fine-tuned models show first non-zero density at $\approx\!0.4375$ units;
pretrained models at $\approx\!0.6875$ (BGE-Base) and $\approx\!0.8125$ (MPNet-Base) units.
...
A separate iterative ablation across three systems shows qualitatively distinct signal
geometries: the human brain ($D_{50}\!=\!4$, AUC 0.302) uses the most distributed code ---
...
Qwen-FT ($D_{50}\!=\!12$, AUC 0.260) compresses signal into a steep cliff;
pretrained MPNet ($D_{50}\!=\!17$, AUC 0.331) spreads it across the widest manifold.
...
\begin{minipage}[t]{0.50\textwidth}
  ...
  \par\vspace{0.2em}
  {\footnotesize (a) Signal cliff: accuracy as dominant directions are removed
  iteratively. Fine-tuned models reach chance at dimensions 15--20;
  pretrained BGE at 26, pretrained MPNet at 67.}
  \phantomsection\label{fig:signal-cliff}
\end{minipage}%
\hfill%
\begin{minipage}[t]{0.48\textwidth}
  ...
  \par\vspace{0.2em}
  {\footnotesize (b) Cross-system context retention.
  Brain $D_{50}\!=\!4$; fine-tuned LLM $D_{50}\!=\!12$;
  pretrained LLM $D_{50}\!=\!17$.}
  \phantomsection\label{fig:context-retention}
\end{minipage}
\refstepcounter{figure}\label{fig:signal-structure}
\par\vspace{0.3em}
\noindent{\small\textbf{Figure~\thefigure: Signal structure across systems (Finding 2).}
(a) SVD ablation reveals qualitatively different compaction strategies.
(b) The brain requires only 4 dominant directions to achieve half its classification
accuracy, yet maintains a distributed plateau rather than collapsing to chance ---
consistent with distributed biological coding.}
\par\vspace{0.5em}
...
\begin{figure}[h!]
  ...
  \caption{\textbf{The Low-Rank Emotion Subspace (Finding 3).}
           ...
           PCA scatter visualisations are in Appendix~\ref{app:scatter}.}
...
Fine-tuning aligns geometric and logit structure by design, so a positive correlation
is expected.
What is not guaranteed by the training objective is the \emph{degree}: a near-linear
relationship ($r\!=\!0.9884$) means centroid margin almost perfectly determines logit
margin, with no residual variance attributable to non-geometric factors.
In overlap regions specifically, logit values are biased toward the competing class
93.4--100\% of the time --- meaning geometry at the decision boundary is fully
determinative of classifier confidence, not merely correlated with it.
This result is reported as a mechanistic characterisation, not a novel discovery:
its value is establishing that the geometric competition framework (centroid margin)
is the operative construct in LLMs, which motivates its application to the brain in
the following section.

\textbf{Brain: centroid margin $\to$ classifier uncertainty.}
Classifier uncertainty is operationalised as $1 - P(\text{correct})$, the complement
of the LOSO classifier's predicted probability for the true emotion class.
For AUC computation, samples are binarised at the median: \emph{high uncertainty} is
defined as $1 - P(\text{correct}) > \text{median}$.
For brain fMRI, raw centroid distance is a weak predictor of this uncertainty
...
The ambiguity-gradient curves exhibit a consistent monotonic structure across systems,
with ambiguity increasing as a function of normalised distance from class centroids.
This qualitative alignment is supported by a full-pipeline permutation test
($n\!=\!5000$ shuffles, $p<0.001$) that destroys cross-system correspondence under
label shuffling.
Correlation coefficients computed over interpolated grid points are not treated as
inferential statistics, as adjacent points are not independent observations;
the permutation test is the sole inferential basis for this comparison.
...
\textbf{(5a) RDM comparison in 48D raw ROI space (Appendix~\ref{app:rdm-figures}).}

\begin{center}
\small
\begin{tabular}{lccc}
\toprule
Comparison & $r$ (direction only)$^\dagger$ & Notes \\
\midrule
...
\bottomrule
\multicolumn{3}{l}{$^\dagger$ $K\!=\!3$, 1 df; $p\!\approx\!0.21$ for $r\!=\!-0.88$; no inferential p-value is valid.}
\end{tabular}
\end{center}
...
the CI spans positive values and cannot support directional inference at this dimensionality).
Brain subjects agree at $r\!\approx\!0.47$, yet the fine-tuned LLM RDM points in the
\emph{opposite direction}; sign and noise-ceiling contrast are load-bearing.
The 11D bootstrap CI ($[-0.9899,\,-0.1178]$, fully negative) provides stronger
directional support (Section~\ref{sec:inversion}).
...
($r\!=\!0.31$).
Because $K$ is small and V-A coordinates are category-level approximations, these
results are global-structure evidence, not primary proof: the dominant neural axis
aligns more strongly with arousal than valence, while the ambiguity mechanism is
supported independently by centroid margin and classifier uncertainty.
...
Both systems exhibit the same relationship: geometric positioning relative to class
prototypes predicts representational certainty --- centroid margin predicts logit margin
in LLMs and classifier uncertainty in the brain.
This follows from the mathematics of distributed representations: any system encoding
discrete categories via distributed activation produces centroids in zero-density space
surrounded by a belt of instances, generating a void and an ambiguity gradient.

\textbf{Statistical hierarchy of evidence.}
\emph{Strongest}: margin-uncertainty coupling (AUC$\,{=}\,0.81$ vs.\ 0.56, feature-shuffle
control, LLM margin-logit $r\!=\!0.99$).
\emph{Strong}: V-A axis inversion, with permutation $p\,{<}\,0.05$ for all three systems.
\emph{Supportive}: 11D RDM inversion (bootstrap CI fully negative).
\emph{Descriptive only}: 48D RDM magnitude (wide CI, $K\!=\!3$) and void--belt density
similarity (Wasserstein $p\!=\!0.46$).

\textbf{The relational paradox: different priorities, not failure.}
The inverted RDM directionality reflects different operational demands, not error.
LLMs encode co-occurrence statistics: ``fear'' and ``sadness'' share negative hedonic
contexts, clustering centroids along valence.
The brain evolved to respond to threats and rewards: fear's high-arousal physiology
differs fundamentally from sadness's low-arousal withdrawal, so arousal is the more
behaviourally predictive axis.

\textbf{Limitations.}
Evidence covers one text emotion dataset (6 classes) and one brain fMRI dataset
(5 emotions, 40 subjects, one ROI atlas).
The 3-emotion cross-system label mapping relies on conceptual equivalence, introducing
uncertainty into RSA comparisons.
V-A coordinates are category-level approximations; global-axis findings are supportive
evidence, not primary proof.
...
ambiguity is shared across LLMs and brain fMRI --- centroid margin predicts logit margin
in LLMs and classifier uncertainty in the brain --- yet global organisation is inverted:
LLMs sort emotions by valence, the brain by arousal.

Three findings have practical value: the low-rank subspace (2.6\% capacity, 20D)
shows efficient emotion encoding does not require large representations;
the margin-vs-distance contrast (AUC 0.81 vs.\ 0.56) provides a reusable diagnostic;
and the V-A inversion suggests training on arousal distinctions may be necessary for
...
\section{V-A Reference Coordinate Robustness}
\label{app:va-sensitivity}

The external V-A reference coordinates used in Section~\ref{sec:inversion} are
category-level approximations from the circumplex literature~\citep{posner2005}.
To confirm that the axis-dominance results are not artefacts of these specific
coordinate assignments, we ran a permutation test ($n\!=\!1000$) for each system.

\textbf{Procedure.}
For each of the three systems (fine-tuned LLM, pretrained LLM, brain fMRI), emotion
centroids are projected into the leading 2D PCA subspace.
Valence and arousal correlations are computed with the reference coordinates
(Table~\ref{tab:va-coords}).
A null distribution is constructed by shuffling the V-A label assignments across
emotions 1,000 times; each shuffle recomputes the maximum of the valence and arousal
correlations.
The permutation $p$-value is the proportion of shuffles that equal or exceed the
...
All three systems reach conventional significance under label permutation.
The axis assignments --- valence-dominant for both LLM types, arousal-dominant for the
brain --- are not recoverable by random reassignment of the reference coordinates.
The qualitative inversion is robust to the specific coordinate values used.
...
\midrule
Individual Brain Charting~\citep{pinho2018}         & 12 & Scientific Data \\
Precision Functional Mapping~\citep{gordon2017}     & 10 & Neuron \\
Natural Scenes Dataset~\citep{allen2022}            & 8  & Nature Neuroscience \\
\textbf{This study (DS005700)}~\citep{openneuro2024}& \textbf{40} & OpenNeuro \\
\bottomrule
\end{tabular}
\end{center}
...
% ---------------------------------------------------------------------------
% ---------------------------------------------------------------------------
\section{Finding 3: PCA Scatter Visualisations}
\label{app:scatter}
% ---------------------------------------------------------------------------
...
% ---------------------------------------------------------------------------
\section{RDM Heatmaps (Finding 5a)}
\label{app:rdm-figures}
% ---------------------------------------------------------------------------
...
\begin{figure}[h!]
  ...
  \caption{\textbf{RDM comparison (Finding 5a).}
           The brain organises affect by arousal intensity while the LLM organises
           by hedonic polarity --- geometries point in opposite directions
           (direction indicator only; $K\!=\!3$, 1 df, $p\!\approx\!0.21$;
           see table for values). Noise ceiling $\approx\!+0.47$.}
  \label{fig:rdms}
\end{figure}
...
\textbf{Code and data.}
Code and data are provided as supplementary material.
