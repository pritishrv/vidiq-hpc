...
The standard $t$-test for $r\!=\!-0.88$ with 1 df gives $p\!\approx\!0.21$ ---
formally non-significant.
No inferential p-value should be attached to these magnitudes.
The load-bearing quantities are sign (direction of inversion) and contrast with the
noise ceiling; the 11D systems-level result (Section~\ref{sec:inversion}) provides
the stronger inferential basis via a bootstrap CI that remains fully negative.
...
\emph{opposite direction} --- sign and contrast with the noise ceiling are the
load-bearing quantities; the exact magnitude carries no inferential weight.
